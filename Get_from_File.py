...
def read_file(fileName):
    ...
    #### Read Jij block
            if Temp[0]=='#' and Temp[1]=='END' and Temp[2]=='INPUT_Jij' :
                Jijflag=False
                if np.shape(JMatrix)[2]!=lenNeihb:
                    raise SyntaxError("Number of neighbour doesn't match")
            
            if Jijflag:
                JMatrix[SubLatticesName[Temp[0]],SubLatticesName[Temp[1]],int(Temp[2])]=float(Temp[3])
            if (Temp[0]=='#' and Temp[1]=='INPUT_Jij'):
                Jijflag = True
                JMatrix = np.zeros([len(SubLatticesName),len(SubLatticesName),int(Temp[2])])
                lenNeihb = int(Temp[2])
    ##### end Jij size block
    ...
    return unitcell, M, SubLattices, S, atoms, ML, qpts,listofEELS, Anisotropies, JMatrix
